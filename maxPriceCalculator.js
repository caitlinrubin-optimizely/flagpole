module.exports = (optlyInstance) => {
  ...
  const isSeaPriceHikeActivated = optlyInstance.isFeatureEnabled('bikini_bottom_price_hike', userId, { location });
  if (isSeaPriceHikeActivated) {
    actualSeaPrice += 100;
  }

  const isLandPriceHikeActivated = optlyInstance.isFeatureEnabled('texas_price_hike', userId, { location });
  ...
  const isSaleActivated = optlyInstance.isFeatureEnabled('sale_promo', userId, { location });
  if (isSaleActivated) {
